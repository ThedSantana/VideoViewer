package ru.medyannikov.videoviewer.ui.custom;
...
public class CustomPlaybackControlView
    extends FrameLayout {
  ...
  public CustomPlaybackControlView(final Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);

    rewindMs = DEFAULT_REWIND_MS; fastForwardMs = DEFAULT_FAST_FORWARD_MS; showTimeoutMs = DEFAULT_SHOW_TIMEOUT_MS;
    if (attrs != null) {
      TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.PlaybackControlView, 0, 0); try {
    ...
    currentWindow = new Timeline.Window(); formatBuilder = new StringBuilder();
    ...
    progressBar.setOnSeekBarChangeListener(componentListener); progressBar.setMax(PROGRESS_BAR_MAX);
    playButton = (ImageView) findViewById(R.id.play); playButton.setOnClickListener(componentListener);
  ...
  private void toggleFullScreen() {
    if (isFullScreen) {
      fullScreenListener.onHideFullScreen();
      isFullScreen = false;
    } else {
      fullScreenListener.onShowFullScreen();
      isFullScreen = true;
    }
    fullScreenButton.setImageResource(isFullScreen ? R.drawable.ic_fullscreen_exit : R.drawable.ic_fullscreen);
  }
  ...
  public void setPlayer(ExoPlayer player) {
    ...
    } this.player = player; if (player != null) {
  ...
  public void setRewindIncrementMs(int rewindMs) {
    this.rewindMs = rewindMs; updateNavigation();
  }
  ...
  public void setFastForwardIncrementMs(int fastForwardMs) {
    this.fastForwardMs = fastForwardMs; updateNavigation();
  }

  /**
   * Returns the playback controls timeout. The playback controls are automatically hidden after this duration of time
   * has elapsed without user input.
   *
   * @return The duration in milliseconds. A non-positive value indicates that the controls will remain visible
   * indefinitely.
   */
  ...
  /**
   * Sets the playback controls timeout. The playback controls are automatically hidden after this duration of time has
   * elapsed without user input.
   *
   * @param showTimeoutMs The duration in milliseconds. A non-positive value will cause the controls to remain visible
   * indefinitely.
   */
  ...
  /**
   * Shows the playback controls. If {@link #getShowTimeoutMs()} is positive then the controls will be automatically
   * hidden after this duration of time has elapsed without user input.
   */
  public void show() {
    if (! isVisible()) {
  ...
  private void hideAfterTimeout() {
    removeCallbacks(hideAction); if (showTimeoutMs > 0) {
      hideAtMs = SystemClock.uptimeMillis() + showTimeoutMs; if (isAttachedToWindow) {
  ...
  private void updateAll() {
    updatePlayPauseButton(); updateNavigation(); updateProgress();
  }
  ...
  private void updatePlayPauseButton() {
    ...
    String contentDescription = getResources().getString(playing
  ...
  private void updateNavigation() {
    ...
    if (haveTimeline) {
      ...
      currentTimeline.getWindow(currentWindowIndex, currentWindow); isSeekable = currentWindow.isSeekable;
  ...
  private void updateProgress() {
    ...
    long position = player == null ? 0 : player.getCurrentPosition(); if (! dragging) {
    ...
    if (playbackState != ExoPlayer.STATE_IDLE && playbackState != ExoPlayer.STATE_ENDED) {
      long delayMs; if (player.getPlayWhenReady() && playbackState == ExoPlayer.STATE_READY) {
        delayMs = 1000 - (position % 1000); if (delayMs < 200) {
          delayMs += 1000;
        }
      } else {
        delayMs = 1000;
  ...
  private void setButtonEnabled(boolean enabled, View view) {
    view.setEnabled(enabled); if (Util.SDK_INT >= 11) {
  ...
  private String stringForTime(long timeMs) {
    if (timeMs == C.TIME_UNSET) {
      timeMs = 0;
    } long totalSeconds = (timeMs + 500) / 1000; long seconds = totalSeconds % 60;
    long minutes = (totalSeconds / 60) % 60; long hours = totalSeconds / 3600; formatBuilder.setLength(0);
    return hours > 0
  ...
  public void onDetachedFromWindow() {
    super.onDetachedFromWindow(); isAttachedToWindow = false; removeCallbacks(updateProgressAction);
    removeCallbacks(hideAction);
  }
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    if (player == null || event.getAction() != KeyEvent.ACTION_DOWN) {
      ...
      case KeyEvent.KEYCODE_MEDIA_FAST_FORWARD: case KeyEvent.KEYCODE_DPAD_RIGHT: fastForward(); break;
      case KeyEvent.KEYCODE_MEDIA_REWIND: case KeyEvent.KEYCODE_DPAD_LEFT: rewind(); break;
      ...
      case KeyEvent.KEYCODE_MEDIA_NEXT: next(); break; case KeyEvent.KEYCODE_MEDIA_PREVIOUS: previous(); break;
  ...
  private final class ComponentListener
    ...
    public void onStartTrackingTouch(SeekBar seekBar) {
      removeCallbacks(hideAction); dragging = true;
    }
    ...
    @Override
    public void onClick(View view) {
      Timeline currentTimeline = player.getCurrentTimeline(); if (playButton == view) {
